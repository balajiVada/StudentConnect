...
async function getCustomizedResources(req, res) {
  try {
    ...
    // Fetch all bookmarked resource IDs for this user
    const userBookmarks = await Bookmark.find({
      user: userId,
      contentModel: 'Resource'
    }).select('contentId').lean();

    const bookmarkedIds = new Set(userBookmarks.map(b => String(b.contentId)));
    ...
    allResources.forEach(resource => {
      ...
      const isSaved = bookmarkedIds.has(String(resource._id));
      ...
      if (match) {
        prioritized.push(resourceObj);
      } else {
        rest.push(resourceObj);
      }
    });
    ...
    console.error('Error in getCustomizedResources:', error);
    res.status(500).json({ error: 'Server error' });
  }
}
